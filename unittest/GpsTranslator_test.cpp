...
TEST(GpsTranslatorTest, esri102020) {
  FEW_TESTS();
  auto logger = buildMemoryLogger(100, LV_DEBUG);
  GpsTranslator translator(*logger, 102020);
// Antarctica: Belgrano II Base
// GPS: -77.874281,-34.7011923
// Esri102020: -769485.2296282591, 1111228.3914792708

  GpsCoordinates gps(-77.874281, -34.7011923);
  EastNorthCoordinates eastWest;
  translator.fromGpsRaw(gps, eastWest);
  ASSERT_STREQ(
      formatCString("%0.2f, %0.2f", eastWest._east, eastWest._north).c_str(),
      "-769485.23, 1111228.39");
  gps.clear();
  eastWest.set(-769485.23, 1111228.39);
  translator.toGpsRaw(eastWest, gps);

  ASSERT_STREQ(
      formatCString("%0.8f, %0.8f", gps._latitude, gps._longitude).c_str(),
      "-77.87428101, -34.70119235");
  delete logger;
}

TEST(GpsTranslatorTest, esri102022) {
  FEW_TESTS();
  auto logger = buildMemoryLogger(100, LV_DEBUG);
  GpsTranslator translator(*logger, 102022);
// Africa: Lome, Airport
// GPS: 6.1704297, 1.2504074
// Esri102022: -2469243.72326684, 717747.3682996938

  GpsCoordinates gps(6.1704297, 1.2504074);
  EastNorthCoordinates eastWest;
  translator.fromGpsRaw(gps, eastWest);
  ASSERT_STREQ(
      formatCString("%0.2f, %0.2f", eastWest._east, eastWest._north).c_str(),
      "-2469243.72, 717747.37");
  gps.clear();
  eastWest.set(-2469243.72, 717747.37);
  translator.toGpsRaw(eastWest, gps);

  ASSERT_STREQ(
      formatCString("%0.8f, %0.8f", gps._latitude, gps._longitude).c_str(),
      "6.17042971, 1.25040743");
  delete logger;
}
